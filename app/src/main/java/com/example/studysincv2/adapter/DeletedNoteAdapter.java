...
import com.example.studysincv2.trash_catatan;

import java.util.List;

public class DeletedNoteAdapter extends RecyclerView.Adapter<DeletedNoteAdapter.DeletedNoteViewHolder> {
    private Context context;
    private List<Note> deletedNoteList;

    public DeletedNoteAdapter(Context context, List<Note> deletedNoteList) {
    ...
    public void onBindViewHolder(@NonNull DeletedNoteViewHolder holder, int position) {
        ...
        if (note.isSelectable()) {
            ...
            holder.checkBoxNote.setOnCheckedChangeListener((buttonView, isChecked) -> {
                ...
                if (isChecked) {
                    ((trash_catatan) context).addSelectedNote(note);
                } else {
                    ((trash_catatan) context).removeSelectedNote(note);
                }
            });
        } else {
        ...
        holder.itemView.setOnLongClickListener(v -> {
            ((trash_catatan) context).startSelectionMode();
            return true;
        });
